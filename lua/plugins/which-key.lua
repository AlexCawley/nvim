return function()
    local wk = require('which-key')

    wk.setup({
        plugins = {
            marks = true,
            registers = true,
            spelling = {
                enabled = true,
                suggestions = 20,
            },
            presets = {
                operators = false,
                motions = false,
                text_objects = false,
                windows = true,
                nav = true,
                z = true,
                g = true,
            },
        },
    })

    wk.add({
        { '<C-h>', '<C-w>h', desc = 'Go to Left Window' },
        { '<C-j>', '<C-w>j', desc = 'Go to Bottom Window' },
        { '<C-k>', '<C-w>k', desc = 'Go to Top Window' },
        { '<C-l>', '<C-w>l', desc = 'Go to Right Window' },
        { '<C-n>', '<cmd>NvimTreeToggle<cr>', desc = 'Toggle File Tree' },
        { '<leader>bl', ':BufferLinePick<CR>', desc = "Pick a buffer" },
        { '<leader>ca', '<cmd>lua vim.lsp.buf.code_action()<cr>', desc = 'Code Actions', mode = { 'n', 'v' } },
        { '<leader>d', group = '.NET' },
        { '<leader>da', function() require('easy-dotnet').add() end, desc = 'Add Package/Project' },
        { '<leader>db', function() require('easy-dotnet').build() end, desc = 'Build Project' },
        { '<leader>dc', function() require('easy-dotnet').clean() end, desc = 'Clean Project' },
        { '<leader>dre', function() require('easy-dotnet').restore() end, desc = 'Restore Packages' },
        { '<leader>drn', function() require('easy-dotnet').run() end, desc = 'Run Project' },
        { '<leader>ds', function() require('easy-dotnet').secrets() end, desc = 'Manage Secrets' },
        { '<leader>dtr', function() require('easy-dotnet').testrunner() end, desc = 'Test Runner' },
        { '<leader>dts', function() require('easy-dotnet').test_solution() end, desc = 'Test Solution' },
        { '<leader>ld', '<cmd>lua vim.diagnostic.open_float()<cr>', desc = 'Show Line Diagnostics' },
        { '<leader>e', '<cmd>NvimTreeFocus<cr>', desc = 'Focus File Tree' },
        { '<leader>fm', function() require('conform').format({ lsp_fallback = true }) end, desc = 'Format File/Selection', mode = { 'n', 'v' } },
        { '<leader>f', group = 'Find' },
        { '<leader>fb', '<cmd>Telescope buffers<cr>', desc = 'Find Buffer' },
        { '<leader>ff', '<cmd>Telescope find_files<cr>', desc = 'Find File' },
        { '<leader>fg', '<cmd>Telescope live_grep<cr>', desc = 'Live Grep' },
        { '<leader>h', group = 'Git Hunks' },
        { '<leader>g', group = 'Git' },
        { '<leader>gs', '<cmd>Git<cr>', desc = 'Git Status' },
        { '<leader>gc', '<cmd>Git commit<cr>', desc = 'Git Commit' },
        { '<leader>gp', '<cmd>Git push<cr>', desc = 'Git Push' },
        { '<leader>gl', '<cmd>Git pull<cr>', desc = 'Git Pull' },
        { '<leader>gb', '<cmd>Git blame<cr>', desc = 'Git Blame' },
        { '<leader>gd', '<cmd>Gdiffsplit<cr>', desc = 'Git Diff Split' },
        { '<leader>gaa', '<cmd>Git add --all<cr>', desc = 'Git Add All' },
        { '<leader>j', '<cmd>split<cr>', desc = 'Split Horizontal' },
        { '<leader>l', '<cmd>vsplit<cr>', desc = 'Split Vertical' },
        { '<leader>nd', '<cmd>lua vim.diagnostic.goto_next()<cr>', desc = 'Next Diagnostic' },
        { '<leader>pd', '<cmd>lua vim.diagnostic.goto_prev()<cr>', desc = 'Previous Diagnostic' },
        { '<leader>x', ':BufferLinePickClose<CR>', desc = 'Close current buffer' },
        { '<C-w>', '<cmd>bprevious | bdelete #<cr>', desc = 'Close Buffer (Keep Window)' },
        { 'K', '<cmd>lua vim.lsp.buf.hover()<cr>', desc = 'Hover Information' },
        { 'gD', '<cmd>lua vim.lsp.buf.declaration()<cr>', desc = 'Go to Declaration' },
        { 'gd', '<cmd>lua vim.lsp.buf.definition()<cr>', desc = 'Go to Definition' },
        { 'gr', '<cmd>lua vim.lsp.buf.references()<cr>', desc = 'Go to References' },
        { '<tab>', ':BufferLineCycleNext<CR>', desc = "Next buffer" },
        { '<S-tab>', ':BufferLineCyclePrev<CR>', desc = "Previous buffer" },
    })
end
